...
if(!isset($_SESSION['name'])){  
    ...
    exit();  }
if($_SESSION['usertype']!='employ'){echo "非管理员";exit;}
?>
<html>
<head>
...
{
header("location:delete.php?num={$_GET['delegoodid']}");
    $stor->delete("goodimg",$_GET['delegoodid']);
echo "<font color='red'>删除成功</font></br>";
}

echo "　　　　　　　　<a href='employ.php' style='position:absolute;top:60;left:220;'><button class='mybtn'>返回主目录</button></a>";
...
echo "</br></br><form action='employ_goods.php' method='post' style='position:absolute;top:20px;left:410px;margin-bottom:0px;'>
...
结束日期：<input type='text' name='endday' style='padding-left:5px;' size='8'  onclick='SelectDate(this)' readonly='readonly' value='{$endday}'/>（结果为起始和结束日期之间记录，不包括当天）</br></br>
...
echo "<option  value='-1'></option>";
 if($_POST['shopid']==0)	echo "<option  value='0' selected='selected'>总库</option>";
 else echo "<option  value='0' >总库</option>";
...
{	
   if($_POST['shopid']==$i)echo "<option  value='$i' selected='selected'>{$row2[1]}</option>";else echo "<option  value='$i'>{$row2[1]}</option>";
    $i++;
}
echo "</select>
<input type='submit' value='查询' name='searchsubmit' /> 
	...
	if(empty($_GET['displayname']))
	{
		...
		//for($i=1;$i<=$shopnumber;$i++)
		{
			$shopstr.=",goods_".$i."_shop";
				...
				if($i==$_POST['shopid']){$str5.="<th style=\'background:#ff0000;\'>{$row8[1]}</th>";	
...
if($pagenum==-1)	$lim="";
////////////////////////////////////////////////////////////////////////////////////////	
		$sql="select goods_flag,goods_id,goods_name,goods_type,goods_brand,goods_color,goods_mark1,goods_buynumber,goods_0_shop".$shopstr.",goods_buyprice,goods_saleprice,goods_controlprice,goods_serialnumber,goods_emp,goods_buytime from goods  
...
if(0==$_POST['shopid'])	{echo "<tr  bgcolor='#A7C942'><th style='border-style:none;background-color:white;width:10;'></th>
		<th>ID</th><th>商品名字</th><th>型号</th><th>品牌</th><th>颜色</th><th>备注</th><th>进货数量</th><th style='background:#ff0000;'>总库余量</th>".$str51."<th>进价</th><th>建议售价</th><th>控价</th><th>商品系列号</th><th>入库店员</th><th>进货日期</th><th>销售商品</th></tr></br>";		
	}
else {echo "<tr  bgcolor='#A7C942'><th style='border-style:none;background-color:white;width:10;'></th>
		<th>ID</th><th>商品名字</th><th>型号</th><th>品牌</th><th>颜色</th><th>备注</th><th>进货数量</th><th style='background:#73B1E0;'>总库余量</th>".$str51."<th>进价</th><th>实际进价</th><th>建议售价</th><th>控价</th><th>商品系列号</th><th>入库店员</th><th>进货日期</th><th>销售商品</th></tr></br>";		
		}
		while($row=mysql_fetch_array($res)) 
		{
			...
			{	
				...
				$str4="<tr ><th>ID</th><th>商品名字</th><th>型号</th><th>品牌</th><th>颜色</th><th>备注</th><th>进货数量</th><th style=\'background:#73B1E0;\'>总库余量</th>".$str5."<th>进价</th><th>建议售价</th><th>控价</th><th>商品系列号</th><th>入库店员</th><th>进货日期</th><th>销售商品</th><th style=\'border-style:none;background-color:red;\'><a  class=\'close\' font-size=5px title=\'关闭\' href=\'javascript:\' onclick=\'closedisplay();\'></a></th></tr>";

				$sql4="select goods_flag,goods_id,goods_name,goods_type,goods_brand,goods_color,goods_mark1,goods_buynumber,goods_0_shop".$shopstr2.",goods_buyprice,goods_saleprice,goods_controlprice,goods_serialnumber,goods_emp,goods_buytime from goods where UNIX_TIMESTAMP(goods_buytime) between UNIX_TIMESTAMP('$startday') and UNIX_TIMESTAMP('$endday') and trim(goods_name)=trim('$good_name') and trim(goods_type)=trim('$good_type')  order by goods_id";
				...
				{	$str7="";	
					...
					$str4=$str4."<tr class=\'mytr\' onclick=\'do_onclick(this)\' onmouseover=\'changeimg({$goodid})\'>".$str7."<th width=120px><a href=\'add_sale_good.php?salegoodid=$goodid\'>销售</a></th><th bgcolor=\'white\'></th></tr></br>";		
				}
				$str4=$str4."</table>";
			...
			for($i=1;$i<$shopnumber+15;$i++)
			{if(($i>7)&&($i<=$shopnumber+8)){if($i==$_POST['shopid']+8)$str6.="<th style='background:#ff0000;'>{$row[$i]}</th>";else $str6.="<th style='background:#73B1E0;'>{$row[$i]}</th>";}
			else	$str6.="<th>{$row[$i]}</th>";	
			}	
			//$str6=htmlspecialchars_decode("</tr></br>");
			
			$str6.="<th width=120px><a href='add_sale_good.php?salegoodid=$goodid'>销售</a>&nbsp;<a href='adjust_goods.php?goodid=$goodid'>调货</a></th></tr></br>";
    ...
    /*echo "<div style='position:absolute;top:105;left:10;'><form action='employ_goods.php' method='post'><a href='employ_goods.php?search_goodname={$searchgoodname}&search_goodtype={$searchgoodtype}&search_goodbrand={$searchgoodbrand}&search_goodcolor={$searchgoodcolor}&page=1' style='cursor:hand;'>首页</a>&nbsp;";
echo "<a href='employ_goods.php?search_goodname={$searchgoodname}&search_goodtype={$searchgoodtype}&search_goodbrand={$searchgoodbrand}&search_goodcolor={$searchgoodcolor}&page=".($pagenum-1)."'>上一页</a>&nbsp;";
...
echo "<a href='employ_goods.php?search_goodname={$searchgoodname}&search_goodtype={$searchgoodtype}&search_goodbrand={$searchgoodbrand}&search_goodcolor={$searchgoodcolor}&page=".($pagenum+1)."'>下一页</a>&nbsp;";
echo "<a href='employ_goods.php?search_goodname={$searchgoodname}&search_goodtype={$searchgoodtype}&search_goodbrand={$searchgoodbrand}&search_goodcolor={$searchgoodcolor}&page=".$maxpage."'>末页</a>&nbsp;";
echo "<a href='employ_goods.php?search_goodname={$searchgoodname}&search_goodtype={$searchgoodtype}&search_goodbrand={$searchgoodbrand}&search_goodcolor={$searchgoodcolor}&page=-1'>所有</a></form></div>";
*/
    echo "<div style='position:absolute;top:105;left:10;'><form action='employ_goods.php' method='post'><a href='admin_goods.php?page=1' style='cursor:hand;'>首页</a>&nbsp;";
echo "<a href='employ_goods.php?page=".($pagenum-1)."'>上一页</a>&nbsp;";
if($pagenum==-1)echo "第1-".$maxpage."页";else echo "第{$pagenum}页/共{$maxpage}页&nbsp;";
echo "<input type='submit' value='每页:'/>&nbsp;<input type='text' style='width:30px' name='onepagenum' value={$onepage} />行&nbsp;";
echo "<a href='employ_goods.php?page=".($pagenum+1)."'>下一页</a>&nbsp;";
echo "<a href='employ_goods.php?page=".$maxpage."'>末页</a>&nbsp;";
echo "<a href='employ_goods.php?page=-1'>所有</a></form></div>";
